...
//中点位移断层 算法
// 此算法出来的效果，跟上面的断层算法不同，呈现出一个高高低低的地形，断层算法的是一个围绕一个方向一直下降高度的地形
// 该算法也称为等离子分形算法和菱形正方形算法
// 尺寸必须是2的幂次方
//--------------------------------------------------------------
...
bool CTERRAIN::MakeTerrainPlasma( int iSize, float fRoughness )
{
	...
	fHeight		  = ( float )iRectSize/2; // 此算法没有控制最大高度和最小高度，直接取高度图的尺寸/2
	...
	while( iRectSize>0 )
	{
		...
		通过对拐角处的值求平均值并添加随机偏移来查找矩形中心的值

		每个找中心，平均值后赋予中心的顶点。一直迭代，把块一直缩小，继续步长下去，效果就是每小块拉起中心的顶点的高度，像一个小山丘
		...
		d = (ni,nj)
		e = (mi,mj)   */
		for( i=0; i<m_iSize; i+=iRectSize )// 由于算法是以4来分割块，所以高度图数据  尺寸必须是2的幂次方
		{
			for( j=0; j<m_iSize; j+=iRectSize )
			{
				...
				fTempBuffer[mi+mj*m_iSize]= ( float )( ( 
					fTempBuffer[i+j*m_iSize] + 
		...
		The height m_heightData.m_ucpData wraps, so we're never left hanging.  The right side of the last
			...
			查找每个矩形左侧和顶部的值。右侧和底部是相邻矩形的左侧和顶部，因此我们不需要计算它们



			高度m_heightData。m_ucpData包装，因此我们永远不会被挂起。行中最后一个矩形的右侧是行中第一个矩形的左侧。列中最后一个矩形的底部是列中第一个矩形的顶部

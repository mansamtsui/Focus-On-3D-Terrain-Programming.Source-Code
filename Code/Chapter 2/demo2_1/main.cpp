...
bool DemoInit( void )
{
	...
	g_glApp.Init( 10, 10, g_iScreenWidth, g_iScreenHeight, 16, "Demo 2_1: Simple Brute Force Implementation", IDI_ICON1, IDR_MENU1 );
	...
	g_bruteForce.LoadHeightMap( "height128.RAW", 128 );
				    ...
				    int		 nCmdShow ) 
{
	...
	while( true )
	{
		...
		//check the status of the menu items
		if( g_glApp.MenuDown( ID_DEMO_EXIT ) )
			PostQuitMessage( 0 );

		//do not update unless active

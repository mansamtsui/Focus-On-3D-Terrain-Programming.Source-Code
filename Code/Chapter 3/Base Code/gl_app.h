...
不得不去寻找最新的glext.h,有了GLEW扩展库，你就再也不用为找不到函数的接口而烦恼，因为GLEW能自动识别你的平台所支持的全部OpenGL高级扩展函数。也就是说，
只要包含一个glew.h头文件，你就能使用gl,glu,glext,wgl,glx的全部函数。glew库提供高版本 gl 函数的支持。
如果不嫌麻烦的话，也可以手写函数指针，来判断各个 opengl 高版本函数是否支持，但是 glew 库 做了大大的简化，使得 opengl 各个版本的函数像原生函数一样，可以随意调用。
*/
#include "glext.h"

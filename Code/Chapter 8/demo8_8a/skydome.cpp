...
void CSKYDOME::GenCloudTexture( int size, float fBlur, float fOctaves, float fAmplitude,
								float fFrequency, float fH, float fOffset )
{
	float* ucpTexData;
	float* fpData;
	float fTemp=0;
	...
	{
		for( x=0; x<size; x++ )
			fpData[( y*size )+x]= FBM( ( float )x, ( float )y, fOctaves, fAmplitude, fFrequency, fH, fOffset );
	}
...
//--------------------------------------------------------------
float CSKYDOME::FBM( float x, float y, float fOctaves, float fAmplitude, float fFrequency, float fH, float fOffset )
{
	float fValue= 0;

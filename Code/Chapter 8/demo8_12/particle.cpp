...
// Return Value: None
//--------------------------------------------------------------
void CPARTICLE_ENGINE::Update( float fTimeStep )
{
	CVECTOR	vecMomentum;
	int		i;
...
//--------------------------------------------------------------
// Name:		 CPARTICLE_ENGINE::CreateRaindrops - public
// Description:	 Make a series of raindrops in the given area
// Arguments:	 -fMinX, fMinY, fMinZ: set the minimum boundaries of the area
//				 -fMaxX, fMaxY, fMaxZ: set the maximum boundaries of the area
//				 -iDropSpeed: speed of the rain drops
//				 -iNumDrops: number of raindrops to be created
// Return Value: None
//--------------------------------------------------------------
void CPARTICLE_ENGINE::CreateRaindrops( float fMinX, float fMinY, float fMinZ,
										float fMaxX, float fMaxY, float fMaxZ, int iDropSpeed, int iNumDrops )
{
	CVECTOR vecEmissionPos;
	int i;

	//store the current emission position
	vecEmissionPos= m_vecPosition;

	for( i=0; i<iNumDrops; i++ )
	{
		//set a new emission position
		SetEmissionPosition( RangedRandom( fMinX, fMaxX ),
							 RangedRandom( fMinY, fMaxY ),
							 RangedRandom( fMinZ, fMaxZ ) );

		//create the rain particle
		CreateParticle( 0.0f, -iDropSpeed, 0.0f );
	}

	//restore the old emission position
	m_vecPosition= vecEmissionPos;
}
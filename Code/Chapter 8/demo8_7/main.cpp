...
bool DemoInit( void )
{
	g_glApp.Init( 10, 10, g_iScreenWidth, g_iScreenHeight, 16, "Demo 8_7: Simple Hardware Fog", IDI_ICON1, IDR_MENU1 );
	...
	g_skydome.Init( 5.0f, 5.0f, 512.0f );
	g_skydome.GenCloudTexture( 256, 0.5f, 25, 1.0f, 0.25f, 0.25f, 0.0f );

	//set the camera's position
	g_camera.SetPosition( 128.0f, g_ROAM.GetTrueHeightAtPoint( 128, 256 ) , 256.0f );
...
{
	float fFogColor[4]= {	0.7f, 0.7f, 0.7f, 1.0f	};
	float ucHeight;
	...
	CLAMP( g_camera.m_vecEyePos[0], 0, g_ROAM.m_iSize );
	CLAMP( g_camera.m_vecEyePos[2], 0, g_ROAM.m_iSize );

	ucHeight= ( float )g_ROAM.GetTrueHeightAtPoint( ( int )g_camera.m_vecEyePos[0], ( int )g_camera.m_vecEyePos[2] );

	if( g_camera.m_vecEyePos[1]<( ucHeight+5 ) )
	...
	g_glApp.BeginTextMode( );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-85, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "Vertices: %d", g_ROAM.GetNumVertsPerFrame( )+g_water.GetNumVertices( )+g_skydome.GetNumVertices( ) );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-100, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "Tris/S:   %d", g_ROAM.GetNumTrisPerFrame( )+g_water.GetNumTriangles( )+g_skydome.GetNumTriangles( ) );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-115, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "MTris/S:  %.3f", ( ( g_ROAM.GetNumTrisPerFrame( )+g_water.GetNumTriangles( )+g_skydome.GetNumTriangles( ) )*g_glApp.GetFPS( ) )/1000000.0f );
	g_glApp.EndTextMode( );

...
bool DemoInit( void )
{
	...
	g_glApp.Init( 10, 10, g_iScreenWidth, g_iScreenHeight, 16, "Demo 8_11: Adding Time-Based Movement and Data Interpolation to a Particle Engine", IDI_ICON1, IDR_MENU1 );
	...
	g_particleEngine.SetMaxLife( 120 );

	//set the particle emitter position
	g_particleEngine.SetEmissionPosition( 0, 0, -50.0f );

	//set the particle color and transparency
	g_particleEngine.SetColor( 1.0f, 0.5f, 0.0f, 1.0f, 0.0f, 0.0f );
	g_particleEngine.SetTranslucency( 1.0f, 0.0f );

	//set the particle size and mass
	g_particleEngine.SetSize( 0.5f, 0.5f, 4.0f, 4.0f );
...
void DemoRender( void )
{
	...
	g_glApp.BeginTextMode( );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-100, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "Tris:   %d", g_particleEngine.GetNumParticlesOnScreen( )*2 );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-115, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "MTris/S:  %.3f", ( ( g_particleEngine.GetNumParticlesOnScreen( )*2 )*g_glApp.GetFPS( ) )/1000000.0f );

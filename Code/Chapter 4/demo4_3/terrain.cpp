	...
	for( z=0; z<m_iSize; z++ )
	{
		for( x=0; x<m_iSize; x++ )
		{
			...
			else if( m_lightingType==SLOPE_LIGHT )
			{
				...
				/*char buffer[64];
				int ret = snprintf(buffer, sizeof(buffer), "%2.2f", fShade * 255);
				char* num_string = buffer;
				g_log.Write(LOG_PLAINTEXT, num_string);*/

				//char存储的int，float，会被转为对应的unicode码，写入文件也是写入unicode码
				////fShade*255 出来的值，转为unicode，n个值连接起来的unicode码，很容易被识别为gb2312的编码，以为两个byte一个字符
				//写入文件后 会被编辑器认为编码是gb2312，无法认为是ansi，一个byte一个字符，自动使用16进制查看
				//但是数据是没问题的，可以逐字节来取出使用

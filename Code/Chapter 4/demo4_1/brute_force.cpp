		...
		for( z=0; z<m_iSize-1; z++ )
		{
			...
			glBegin( GL_TRIANGLE_STRIP );
				...
				for( x=0; x<m_iSize-1; x++ )
				{
					...
					ucShadeLow = GetTrueHeightAtPoint( x, z );
					ucShadeHigh= GetTrueHeightAtPoint( x, z+1 );//下一行的高度值就是高光的光照值

					//set the color with OpenGL, and render the point
					//通过设置顶点颜色来控制光照颜色 高度值0-255 所以灯光效果值 范围为 0（暗）到 255（亮）。
					//这种设置灯光效果，就是逐顶点光照
					//光照强度 = 阴影 * 灯光颜色  255 * 0.5  设置到顶点颜色中，也是控制在0-255

					/*
					这正是基于高度的照明所做的：高区域是明亮的，而低区域是黑暗的。
					这种做法是不科学，不符合现实
					*/
			...
			for( z=0; z<m_iSize-1; z++ )
			{
				...
				glBegin( GL_TRIANGLE_STRIP );
					...
					for( x=0; x<m_iSize-1; x++ )
					{
						...
						//get the brightness for the two points to be rendered
						ucShadeLow = GetTrueHeightAtPoint( x, z );
						ucShadeHigh= GetTrueHeightAtPoint( x, z+1 );

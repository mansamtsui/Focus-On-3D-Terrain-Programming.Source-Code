...
bool DemoInit( void )
{
	...
	g_glApp.Init( 10, 10, g_iScreenWidth, g_iScreenHeight, 16, "Demo 5_1: The Basics of Geomipmapping", IDI_ICON1, IDR_MENU1 );
	...
	g_geomipmapping.MakeTerrainFault( 257, 64, 0, 255, 0.05f );
	...
	g_geomipmapping.DoDetailMapping( g_bDetail, 16 );

	//make the texture map, and then save it
	g_geomipmapping.GenerateTextureMap( 256 );
	g_geomipmapping.DoTextureMapping( g_bTexture );
	...
	g_camera.SetPosition( 64.0f, 1000.0f, 256.0f );
	g_camera.m_fYaw+= 130;
...
void DemoRender( void )
{
	...
	glPushMatrix( );
		g_geomipmapping.Scale( 8.0f, 3.0f, 8.0f );
	...
	g_glApp.BeginTextMode( );
		...
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-100, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "MTris/S:  %.3f", ( g_geomipmapping.GetNumTrisPerFrame( )*g_glApp.GetFPS( ) )/1000000.0f );

		//render the number of terrain pathes rendered per frame
		g_glApp.Print( g_iScreenWidth-175, g_iScreenHeight-130, CVECTOR( 0.0f, 1.0f, 0.0f),
					   "Patches:  %d", g_geomipmapping.GetNumPatchesPerFrame( ) );
		...
		if( g_bDetail )
			g_glApp.Print( 0, g_iScreenHeight-85, CVECTOR( 0.0f, 1.0f, 0.0f), "Detail Mapping: Enabled", g_glApp.GetFPS( ) );
		else
			g_glApp.Print( 0, g_iScreenHeight-85, CVECTOR( 0.0f, 1.0f, 0.0f), "Detail Mapping: Disabled", g_glApp.GetFPS( ) );
	g_glApp.EndTextMode( );
			...
			switch( LOWORD( wParam ) )
				{
					case ID_SLOPE_LIGHT_OK:
						...
						g_geomipmapping.SetLightingType( SLOPE_LIGHT );
						g_geomipmapping.SetLightColor( vecColor );
						g_geomipmapping.CustomizeSlopeLighting( iDirX, iDirZ,
			...
			switch( LOWORD( wParam ) )
				{
					case ID_ROUGHNESSDD_GENERATE:
						...
						if( g_iFractalAlgo==FAULT_FORMATION )
							g_geomipmapping.MakeTerrainFault( 257, 64, 0, 255, fDetail );

						else if( g_iFractalAlgo==MIDPOINT_DISPLACEMENT )
							g_geomipmapping.MakeTerrainPlasma( 128, fDetail );
			...
			switch( LOWORD( wParam ) )
				{
					case ID_TEXTUREDD_GENERATE:
						...
						g_geomipmapping.GenerateTextureMap( uiSize );
				    ...
				    int		 nCmdShow ) 
{
	...
	while( true )
	{
		...
		{
			sprintf( szBuffer, "height%d_%d.RAW", g_geomipmapping.m_iSize, g_iCurrentHeightmap );

			g_geomipmapping.SaveHeightMap( szBuffer );
		...
		//save the current texture map
			...
			g_geomipmapping.SaveTextureMap( "texturemap.bmp" );
		...
		//save the current light map to a file
			...
			g_geomipmapping.SaveLightMap( "lightmap.raw" );
